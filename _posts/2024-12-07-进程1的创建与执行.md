...
`getblk`函数的核心目的是获取一个缓冲块，它会优先从缓冲区中查找是否已经存在这样的缓冲块，这里是调用`get_hash_table`函数实现的。如果已经存在了这样的缓冲块就直接返回它即可。如果不存在这样的缓冲块，就从缓冲区空闲列表中分配一个空闲块并返回，具体来说，就是遍历`free_list`链表，选择一个合适的缓冲块并返回。（不要陷入代码细节）
